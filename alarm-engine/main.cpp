...
AppConfig parse_args(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        else if (arg == "--seed") cfg.seed_rules = true;
...
void seed_default_rules(AlarmStore& store) {
    ...
    Rule rules[] = {
        {0, "pump-001", "temperature", Condition::GreaterThan, 80.0,
         Severity::Warning, "{device} {sensor} ({value}) exceeded threshold {threshold}"},
        {0, "pump-001", "temperature", Condition::GreaterThan, 95.0,
         Severity::Critical, "{device} {sensor} CRITICAL ({value}) exceeded {threshold}"},
        {0, "pump-001", "pressure", Condition::LessThan, 1.0,
         Severity::Critical, "{device} pressure critically low ({value} < {threshold})"},
        {0, "pump-001", "pressure", Condition::GreaterThan, 4.5,
         Severity::Warning, "{device} pressure high ({value} > {threshold})"},
        {0, "pump-001", "vibration", Condition::GreaterThan, 10.0,
         Severity::Warning, "{device} vibration high ({value} > {threshold})"},
        {0, "compressor-001", "temperature", Condition::GreaterThan, 90.0,
         Severity::Warning, "{device} {sensor} ({value}) exceeded {threshold}"},
        {0, "compressor-001", "temperature", Condition::GreaterThan, 105.0,
         Severity::Critical, "{device} {sensor} CRITICAL ({value}) exceeded {threshold}"},
        {0, "compressor-001", "current", Condition::GreaterThan, 25.0,
         Severity::Warning, "{device} current high ({value} > {threshold})"},
    };

    int count = 0;
    for (const auto& r : rules) {
        if (store.add_rule(r)) ++count;
    }
    spdlog::info("Seeded {} default rules", count);
}

std::vector<ingestion::Reading> fetch_readings_since(
    PGconn* conn, const std::string& since) {
...
int main(int argc, char* argv[]) {
    ...
    spdlog::info("Alarm engine started with {} rules (poll every {}ms)",
                 rules.size(), cfg.poll_interval_ms);

    std::string last_check;
    {
        auto* res = PQexec(conn,
            "SELECT COALESCE(MAX(timestamp)::text, '1970-01-01T00:00:00Z') "
            "FROM readings");
    ...
    while (g_running.load()) {
        ...
        for (const auto& r : readings) {
            auto alarms = evaluator.evaluate_all(
                rules, r.device_id, r.sensor, r.value, r.timestamp);
            for (const auto& a : alarms) {
                if (store.write_alarm(a)) ++alarm_count;
            }
            last_check = r.timestamp;
        }

        if (alarm_count > 0) {
            spdlog::info("Evaluated {} readings, triggered {} alarms",
                         readings.size(), alarm_count);
        }
    }
